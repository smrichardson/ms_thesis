...
\newpage
\begin{abstract}
This thesis introduces RATE (Rewrite-based Attribute Treatment Estimators), a novel statistical method for evaluating reward models in language modeling. Reward models are functions that score responses based on their quality for a given prompt, but they often serve as imperfect proxies for actual preferences. A critical concern is whether these models genuinely capture intended attributes (e.g., helpfulness) or merely recognize correlated proxies (e.g., response length).

Our key contribution is a causal framework that measures the isolated effect of specific attributes on reward values. RATE leverages large language models to generate counterfactual texts that differ only in the target attribute, allowing us to disentangle causal effects from confounding factors. By employing a double-rewrite technique, we correct for imperfections in the rewriting process, yielding unbiased and $\sqrt{n}$-consistent estimates of treatment effects. We provide theoretical guarantees and empirical validation of RATE's effectiveness.
...
\textbf{A Naive Approach.} One might try to isolate the impact of an attribute $W$ by simply partitioning the data into $W=1$ and $W=0$ examples and comparing mean reward. However, if $W$ is correlated with other features of the text, that naive approach will conflate multiple causal effects. For example, if helpful responses tend to be longer, the naive approach would suggest that helpfulness drives reward, even supposing that the reward model is actually indifferent to helpfulness but merely prefers longer responses.

\subsection{Our Contribution}
To properly estimate how the reward would change if we were to \emph{intervene} on one specific text attribute while keeping other text aspects fixed, we adopt a formal causal perspective.

\textbf{The primary contribution of this thesis is RATE (Rewrite-based Attribute Treatment Estimators)}, a novel framework with three key innovations:
\begin{enumerate}
\item A \textbf{causal formulation} of reward model evaluation that isolates attribute effects from confounders
\item A \textbf{principled rewriting methodology} using LLMs to create approximate counterfactual texts
\item A \textbf{double-rewrite correction technique} that accounts for imperfections in LLM rewrites
\end{enumerate}

If we were able to generate perfect counterfactuals, causal estimation would be straightforward. However, LLM-based rewrites are typically \emph{imperfect}, and may inadvertently alter additional attributes. Our key technical insight is that by rewriting \emph{twice} and comparing the resulting reward differences, we can correct for these off-target changes. We prove that under mild assumptions, this procedure yields \emph{unbiased,} $\sqrt{n}$-consistent estimates of the average treatment effect (ATE), average treatment effect on the treated (ATT), and average treatment effect on the untreated (ATU).
...
Overall, RATE can be viewed as an extension of text-oriented causal inference methods to understand reward models.
...
Our derivations hold for both, but we focus on the pointwise version to keep notation simple. A reward model may be prespecified, in which case we do not need to use explainability methods to understand its behavior. However, in many cases, the reward model is a complex, opaque function (e.g., a neural network) that we cannot easily interpret. In such cases, we may use explainability methods to understand what the model is actually rewarding. Our key motivating application is reinforcement learning from human preferences, where the reward model is trained to predict human preferences from pairwise comparisons. In this framework, an LLM is presented with a prompt $x$ and produces for each prompt a pair of responses $y_1, y_0$. Human raters are then asked to compare the two responses. Without loss of generality we can call $y_1$ the preferred response, and $y_0$ the non-preferred response. We posit a statistical model according to which these preferences are driven by some oracle reward function mapping prompt-response pairs to scalar values, $R^*(x, y): \mathcal{X} \times \mathcal{Y} \to \mathbb{R}$. That is, we model $\text{Pr}(y_1 > y_0) = \phi(R^(x, y_1), R^*(x, y_0))$ where $\phi$ is some function mapping pointwise rewards to preference probabilities.
...
However, in high-dimensional text settings, this approach faces critical barriers:
...
\textbf{Challenge:} LLM rewrites can inadvertently alter off-target attributes (e.g., grammar, length, writing style) beyond the target attribute $W$. Let $\epsilon$ represent the difference in reward caused by these unintended changes. A naive comparison $R(x, y) - R(x, \text{Re}(x,y,w_{\text{new}}))$ would capture both the effect of changing $W$ and these unintended modifications.
...
This process creates a ``rewrite-of-rewrite'' that shares the same rewriting artifacts as the first rewrite but has the original attribute value. By comparing these two texts, we isolate the effect of the attribute change.
...
The rewriting artifacts introduced in steps 2 and 3 will be similar in distribution, allowing them to cancel out in expectation.
...
\textbf{LLM-Based Rewriting.} We implement RATE using modern LLMs (e.g., GPT-4) with carefully crafted prompts. For each sample $(x^i, y^i, W(x^i, y^i))$, we follow a structured rewriting protocol:
...
\begin{enumerate}
    \item \textbf{A formal causal framework} for analyzing reward model behavior, separating true attribute effects from confounding factors
